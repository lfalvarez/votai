...
class FormsTestCase(MediaNaranjaAdaptersBase):
    ...
    def test_proposals_form_instanciate(self):
        ...
        self.assertIn(str(self.p1), possible_labels[self.p1.id])
        self.assertIn(str(self.p2), possible_labels[self.p2.id])
        self.assertIn(str(self.p3), possible_labels[self.p3.id])
...
class MediaNaranjaWizardFormTestsBase(MediaNaranjaAdaptersBase):
    ...
    def complete_wizard(self):
        ...
        for i in range(steps.count):
            ...
            data = {'media_naranja_wizard_form-current_step': str(i)}
